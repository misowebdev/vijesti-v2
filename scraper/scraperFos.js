import { PodaciFos } from "../models/ubaciPodatke.js";
...
const scraperFos = async (page) => {
  ...
  await page.waitForSelector("ul.slider.animated > li:nth-child(4) > a > span > img");
  await skrol(page);
  ...
  let data = await page.evaluate(() => {
    if (skrolovano) {
      ...
      for (let i = 0; i < triReda.length; i++) {
        let naslovidata = triReda[i].querySelectorAll("li.slide div.naslov, div.sadrzaj > span, span.title, span.big-card__naslov");
        let slikedata = triReda[i].querySelectorAll("img.slika, span.slika-container > img, img.big-card__img, img.leaderboard-card__image");
        let linkovidata = triReda[i].querySelectorAll("li.slide a, a.smaller-item, a.big-card.tekst, a.leaderboard-card");
